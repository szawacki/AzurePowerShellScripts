...
Function Download-FileFromWebApp
{
    <#

    .SYNOPSIS

    Download a file from azure web apps via kudu engine.


    .DESCRIPTION


    This function downloads a file from azure web app via the kudu engine. The kudu path can be outside wwwroot. 
    ...
    The path to download the file from. Paths outside wwwroot are possible.


    .PARAMETER LocalPath

    The full qualified path on your local system to download the file to.


    .EXAMPLE 

    Download a file from wwwroot.

    Download-FileFromWebApp -WebAppName "DemoWebApp" -ResourceGroupName "DemoWebAppRG" -KuduPath "site/wwwroot/bin/test.json" -LocalPath "C:\tmp\test.json"


    .EXAMPLE 

    Download a file outside wwwroot.

    Download-FileFromWebApp -WebAppName "DemoWebApp" -ResourceGroupName "DemoWebAppRG" -KuduPath "data/functions/test.json" -LocalPath "C:\tmp\test.json"
...
Download-FileFromWebApp -WebAppName $webAppName -ResourceGroupName $ResourceGroupName -KuduPath $KuduFilePath -LocalPath $LocalFilePath
